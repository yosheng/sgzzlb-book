...
| 战法点评 | 进阶版屠几上肉。非常适合袁术。不过比起形机，大多核弹袁术队都会带上破阵、万箭、横扫之类的AOE高伤战法。再来就是白板袁术属性不高，除非三红以上不怎么建议玩。也非常适合其他文武双全的武将如天水弓/麒麟弓的姜维和三皇肉骑的刘备等。 |
| 实用度 | ★★★★ |
...
| 战法点评 | 非常特殊的一个战法，专门克制拥有特技的装备（除了战斗准备回合就发动的特技如奸雄和无惧）。所以能不能第一回合发动就成了这个战法的关键，毕竟大多特技都是在前两回合生效的。因此，这个战法需要给速度快的武将拿到先手才能发挥作用不然被爆头了再禁用装备就没什么意义了。就算对方装备没有特技也可以削弱敌人属性（四件装备的属性加成几乎相当于进阶2-3红了）。至于水攻也可以搭配张角触发震慑状态或者庞统的铁索连环。 |
| 实用度 | ★★★★ |
...
| 战法点评 | 援护类似嘲讽只不过是加在自身的状态，不会被净化也不会被洞察免疫。不过出场率不怎么高，特殊情况有可能会用上吧比如工具人帮忙抗伤害或者玩反击流的武将如夏侯惇、皇莆嵩等实在没有别的战法的情况下可以暂时用着。 |
| 实用度 | ★★★ |
...
| 战法点评 | AOE控制后输出是个蛮好的战法。不过发动率偏低还要准备1回合，稳定性不怎么高。而且虚弱状态只有1回合很难触发猛击。 |
| 实用度 | ★★★ |
...
| 战法点评 | 无法普攻不过可以增加前期生存力和后期的输出。适合普通攻击没什么伤害的智将如诸葛亮、司马懿ˢ²等也可以用来保护输出高的智将如陆逊。可以搭配夺魂挟魄不过脸黑的话第4回合没有吃到夺魂的BUFF就会很可惜。只有一张吕蒙不拆除非您没想要玩嘟嘟队（笑），不过S2司马盾已经成型而且没嘟嘟的话可以先拆。 |
| 实用度 | ★★★★★ |
...
| 战法点评 | 进阶版自愈。适合肉盾武将。除了让夏侯惇带着开荒或者给董卓自爆队的副将带外，没什么出场机会。徐晃有重复卡再拆。袁绍千万别拆（S1可以组平民群攻、S2可以换鬼神、S3可以换三势阵）。 |
| 实用度 | ★★★ |
...
| 战法点评 | 保护脆皮核心武将的神战法。什么队伍都可以用。庞德直接拆！徐庶可以留到S2组蜀智。 |
| 实用度 | ★★★★★ |
...
[【心得】戰法一覽【S1-S9】](https://forum.gamer.com.tw/C.php?bsn=36815&snA=1819)
```
